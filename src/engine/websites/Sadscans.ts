...
function MangaInfoExtractor(element: HTMLDivElement) {
    return {
        ...
        title: element.querySelector<HTMLHeadingElement>('h2').textContent.trim()
    };
}

@Common.MangaCSS(/^{origin}\/series\/[^/]+$/, 'div.title h2')
@Common.MangasSinglePageCSS('/series', 'div.series-list', MangaInfoExtractor)
@Common.ChaptersSinglePageCSS('div.chap-link div.link a', undefined, Common.AnchorInfoExtractor(true))
