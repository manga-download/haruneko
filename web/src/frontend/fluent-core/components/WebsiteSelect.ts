...
const template = html<WebsiteSelect>`
    ...
    <div id="dropdown" ${ref('dropdown')}>
        ...
        <fluent-lazy-scroll id="entries" :Items=${model => model.filtered} :Pagination=${100} :template=${model => CreateItemTemplate(model.SelectEntry.bind(model))}></fluent-lazy-scroll>
    </div>
`;
