...
// TODO : Revise this website once 'credential: omit' is fixed
...
const chapterScript = `
    new Promise( async (resolve, reject) => {
        try {
            ...
            resolve( data.c.map( chapter =>  {
                return { id: '/lecture-en-ligne/'+ chapter.u,title: chapter.nt };
            }));
...
@Common.MangaCSS(/^{origin}\/\d+(-\d+)?\/[^.]+\.html$/, 'div.h2_titre h2')
