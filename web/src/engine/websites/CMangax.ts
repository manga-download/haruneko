...
type APIMangaDetails = APIResult<APIMedia>;
...
export default class extends DecoratableMangaScraper {
    ...
    public override async GetChapterURL(chapter: Chapter): Promise<URL> {
        const { data: { info } } = await FetchJSON<APIMangaDetails>(new Request(new URL(`./get_data_by_id?id=${chapter.Parent.Identifier}&table=album&data=info`, this.ResourceURL)));
        const { url } = <APIMediaInfos>JSON.parse(info);
        return new URL(`/album/${url}/chapter-${chapter.Title.match(/\d+$/).at(0)}-${chapter.Identifier}`, this.URI);
    }
}