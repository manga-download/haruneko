...
export default class extends DecoratableMangaScraper {
    ...
    public override async FetchPages(chapter: Chapter): Promise<Page[]> {
        ...
        switch (type) {
            case 'chapter': {
                const { pages } = await this.FetchViewer<ChapterViewerViewV3>(type, mangaId, chapterId, secretKey);
            ...
            case 'gravure': {
                let data = await this.FetchViewer<GravureViewerViewV3>(type, mangaId, chapterId, secretKey);
                if (!data.images) data = await this.FetchViewer<GravureViewerViewV3>(type, mangaId, chapterId, secretKey, true);
                if (data.images) {
            ...
            case 'magazine': {
                let data = await this.FetchViewer<ZebrackResponse>(type, mangaId, chapterId, secretKey);
                if (!data.magazineViewerView) data = await this.FetchViewer<ZebrackResponse>(type, mangaId, chapterId, secretKey, true);
            ...
            case 'volume': {
                let data = await this.FetchViewer<VolumeViewerViewV3>(type, mangaId, chapterId, secretKey);
                if (!data.pages) data = await this.FetchViewer<VolumeViewerViewV3>(type, mangaId, chapterId, secretKey, true);
                if (data.pages) {
    ...
    private async FetchViewer<T extends JSONElement>(type: string, mangaId: string, issueId: string, secretKey: string, isTrial: boolean = false): Promise<T> {
        ...
        switch (type) {
            ...
            case 'chapter': {
                ...
                }), protoTypes, 'Zebrack.ChapterViewerViewV3');
            };
            ...
            case 'volume': {
                ...
                return await FetchProto<T>(new Request(url), protoTypes, 'Zebrack.VolumeViewerViewV3');
            };
            ...
            case 'magazine': {
                ...
                url.search = searchParams.toString();
                return await FetchProto<T>(new Request(url), protoTypes, this.responseRootType);
            };
            ...
            case 'gravure': {
                ...
                url.search = searchParams.toString();
                return await FetchProto<T>(new Request(url), protoTypes, 'Zebrack.GravureViewerViewV3');
            };
        }
    }
...
function XORDecrypt(encrypted: Uint8Array<ArrayBuffer>, key: string): Uint8Array {
    if (key) {
