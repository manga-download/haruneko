...
export default class extends DecoratableMangaScraper {
    ...
    public override async FetchPages(chapter: Chapter): Promise<Page[]> {
        ...
        (await FetchCSS<HTMLInputElement>(new Request(redirect), 'div#meta input')).forEach(element => metadatas.set(element.name, element.value));

        const authkey = metadatas.get('param');
        const endpoint = new URL(metadatas.get('cgi'), this.URI).href;
