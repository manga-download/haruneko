...
export default class extends DecoratableMangaScraper {
    ...
    public override async FetchPages(chapter: Chapter): Promise<Page[]> {
        const referer = new URL(chapter.Identifier, this.URI);
        ...
        return [...new Array(maxpage).keys()].map(index => {
            return new Page(this, chapter, new URL(pageone.replace(/(\d+)$/, String(index + 1).padStart(3, '0')), this.URI), { 'Referer': referer.href });
        });
    }
}
