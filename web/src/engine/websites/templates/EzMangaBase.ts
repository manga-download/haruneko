...
export class EzMangaBase extends DecoratableMangaScraper {

    private apiURL: string = undefined;

    public WithApiURL(url: string): EzMangaBase {
        this.apiURL = url;
        return this;
    }
    ...
    public override async FetchChapters(manga: Manga): Promise<Chapter[]> {
        ...
        return Array.fromAsync(async function* (this: This) {
            for (let page = 1, run = true; run; page++) {
                const { data } = await FetchJSON<APIResult<APIChapter[]>>(new Request(new URL(`./series/${manga.Identifier}/chapters?perPage=100&page=${page}&sort=desc`, this.apiURL)));
