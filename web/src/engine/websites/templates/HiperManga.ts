...
@Common.ImageAjax()
export class HiperManga extends DecoratableMangaScraper {

    private readonly apiURL: string = `${this.URI.origin}/api/trpc/`;
    private secretHeader: APIHeader;
    private useSecretHeader: boolean = true;

    public override async Initialize(): Promise<void> {
        if (!this.useSecretHeader) return super.Initialize();
    ...
    public WithSecretHeader(useToken: boolean) {
        this.useSecretHeader = useToken;
        return this;
    }
    ...
    private async FetchTRPC<T extends JSONElement>(endpoint: string, payload: JSONElement): Promise<T> {
        ...
        const [{ result: { data: { json } } }] = await FetchJSON<APIResult<T>>(new Request(new URL(uri), {
            headers: {
                ...this.secretHeader && { [this.secretHeader.key]: this.secretHeader.value },
