...
@Common.MangaCSS(/^{origin}\/comic\/[^/]+$/, 'div.manga-right h1.manga-title', (head, uri) => ({
    id: uri.pathname,
    title: head.innerText.replace(/Comic Details$/, '').trim(),
}))
@Common.ChaptersSinglePageCSS<HTMLAnchorElement>('ul.basic-list li a', undefined, anchor => ({
