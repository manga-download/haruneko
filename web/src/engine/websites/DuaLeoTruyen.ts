...
function PageExtractor(img: HTMLImageElement): string {
    ...
    try {
        ...
        const decryptedBase = DecryptXOR(GetBytesFromURLBase64(basename), GetBytesFromUTF8('dualeo_salt_2025'));
        const filename = `${GetUTF8FromBytes(decryptedBase)}.${extension}`;
...
@Common.MangasMultiPageCSS<HTMLAnchorElement>('div.li_truyen > a:has(img)', Common.PatternLinkGenerator('/truyen-hoan-thanh?page={page}'), 0, anchor => ({
    id: anchor.pathname,
