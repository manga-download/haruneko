...
export default class extends DecoratableMangaScraper {
    ...
    public override async FetchMangas(provider: MangaPlugin): Promise<Manga[]> {
        ...
        const data = await FetchWindowCSS<HTMLAnchorElement>(new FetchRequest(url), 'div#d_list_page a:nth-last-child(2)');
        ...
        for (let page = 0; page < pageMax; page += 1) {
            ...
            const data = await FetchWindowCSS<HTMLAnchorElement>(new FetchRequest(url), this.queryManga);//If not using FetchWindowCSS we always fetch the same page
