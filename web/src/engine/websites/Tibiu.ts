...
export default class extends DecoratableMangaScraper {
    ...
    private async GetMangasFromPage(page: number, provider: MangaPlugin): Promise<Manga[]> {
        const { data } = await FetchJSON<APIResult<APIMediaItem[]>>(new Request(new URL(`./data/comic?page=${page}`, this.apiUrl)));
    ...
    public override async FetchChapters(manga: Manga): Promise<Chapter[]> {
        ...
        const { data } = await FetchJSON<APIResult<APIMediaItem[]>>(new Request(new URL(`./data/chapter?mid=${mangaId}`, this.apiUrl)));
