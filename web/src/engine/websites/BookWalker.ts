  ...
  new Promise(async (resolve, reject) => {
     try {
         ...
         const pages = content.configuration.contents.map((page, index) => {
             ...
             fPage.z4F(content); // function names depends on publus version

             let blocks = [];
             if (fileinfos.BlockHeight) //if we have a block size for the page, its a puzzle !
             {
                 mode = 'puzzle';
                 blocks = window.NFBR.a6G.a5x.prototype.S3g(fPage, fPage.width, fPage.height);
             }
...
export default class extends DecoratableMangaScraper {
    ...
    public constructor() {
        super('bookwalker', `BookWalker`, 'https://global.bookwalker.jp' /*, Tags.Language.English, Tags ... */);
    }
