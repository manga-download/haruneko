// md5.ts
export function MD5(bytes: Uint8Array): string {
    const bitLen = bytes.length * 8;

    const data = new Uint8Array(bytes.length + 9 + 63 & ~63);
    data.set(bytes);
    data[bytes.length] = 0x80;

    const dv = new DataView(data.buffer);
    dv.setUint32(data.length - 8, bitLen, true);

    let a = 0x67452301;
    let b = 0xefcdab89;
    let c = 0x98badcfe;
    let d = 0x10325476;

    const R = (x: number, n: number) => x << n | x >>> 32 - n;
    const A = (x: number, y: number) => x + y >>> 0;
    const C = (q: number, a: number, b: number, x: number, s: number, t: number) =>
        A(R(A(A(a, q), A(x, t)), s), b);

    const F = (a: number, b: number, c: number, d: number, x: number, s: number, t: number) =>
        C(b & c | ~b & d, a, b, x, s, t);

    const G = (a: number, b: number, c: number, d: number, x: number, s: number, t: number) =>
        C(b & d | c & ~d, a, b, x, s, t);

    const H = (a: number, b: number, c: number, d: number, x: number, s: number, t: number) =>
        C(b ^ c ^ d, a, b, x, s, t);

    const I = (a: number, b: number, c: number, d: number, x: number, s: number, t: number) =>
        C(c ^ (b | ~d), a, b, x, s, t);

    const K = new Uint32Array([
        0xd76aa478, 0xe8c7b756, 0x242070db, 0xc1bdceee, 0xf57c0faf, 0x4787c62a, 0xa8304613, 0xfd469501,
        0x698098d8, 0x8b44f7af, 0xffff5bb1, 0x895cd7be, 0x6b901122, 0xfd987193, 0xa679438e, 0x49b40821,
        0xf61e2562, 0xc040b340, 0x265e5a51, 0xe9b6c7aa, 0xd62f105d, 0x02441453, 0xd8a1e681, 0xe7d3fbc8,
        0x21e1cde6, 0xc33707d6, 0xf4d50d87, 0x455a14ed, 0xa9e3e905, 0xfcefa3f8, 0x676f02d9, 0x8d2a4c8a,
        0xfffa3942, 0x8771f681, 0x6d9d6122, 0xfde5380c, 0xa4beea44, 0x4bdecfa9, 0xf6bb4b60, 0xbebfbc70,
        0x289b7ec6, 0xeaa127fa, 0xd4ef3085, 0x04881d05, 0xd9d4d039, 0xe6db99e5, 0x1fa27cf8, 0xc4ac5665,
        0xf4292244, 0x432aff97, 0xab9423a7, 0xfc93a039, 0x655b59c3, 0x8f0ccc92, 0xffeff47d, 0x85845dd1,
        0x6fa87e4f, 0xfe2ce6e0, 0xa3014314, 0x4e0811a1, 0xf7537e82, 0xbd3af235, 0x2ad7d2bb, 0xeb86d391
    ]);

    const S = [7, 12, 17, 22, 5, 9, 14, 20, 4, 11, 16, 23, 6, 10, 15, 21];

    for (let i = 0; i < data.length; i += 64) {
        const X = new Uint32Array(16);
        const block = new DataView(data.buffer, i, 64);

        for (let j = 0; j < 16; j++) {
            X[j] = block.getUint32(j * 4, true);
        }

        let A0 = a, B0 = b, C0 = c, D0 = d;

        for (let j = 0; j < 64; j++) {
            const round = j >> 4;
            const g = [j, (5 * j + 1) % 16, (3 * j + 5) % 16, 7 * j % 16][round];
            const fn = [F, G, H, I][round];
            const s = S[round * 4 + j % 4];

            const tmp = fn(a, b, c, d, X[g], s, K[j]);
            a = d; d = c; c = b; b = tmp;
        }

        a = A(a, A0);
        b = A(b, B0);
        c = A(c, C0);
        d = A(d, D0);
    }

    const hex = (n: number) => ("00000000" + n.toString(16)).slice(-8);
    return (hex(a) + hex(b) + hex(c) + hex(d)).toLowerCase();
}
