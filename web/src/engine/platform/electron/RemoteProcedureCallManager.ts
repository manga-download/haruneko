...
import { MainChannels } from '../../../../../app/electron/src/ipc/RemoteProcedureCallManager';
...
export default class RemoteProcedureCallManager {
    ...
    constructor(private readonly ipc: IPC<MainChannels, never>, settingsManager: SettingsManager) {
    ...
    public async Stop(): Promise<void> {
        return this.ipc.Send(MainChannels.Stop);
    }

    public async Restart(port: number, secret: string): Promise<void> {
        return this.ipc.Send(MainChannels.Restart, port, secret);
    }
}