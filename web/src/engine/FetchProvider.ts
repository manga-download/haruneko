...
let fetchGraphQL: <TResult>(request: FetchRequest, operationName: string, query: string, variables: JSONElement) => Promise<TResult> = fail;
export function FetchGraphQL<TResult>(request: FetchRequest, operationName: string, query: string, variables: JSONElement): Promise<TResult> {
