...
export async function FetchGraphQL<TResult>(request: FetchRequest, operationName: string, query: string, requestVariables: string = undefined): Promise<TResult> {

    const graphQLRequest = new FetchRequest(request.url, {
        body: JSON.stringify({ operationName, query, variables: requestVariables ? requestVariables: {} }),
        method: 'POST',
